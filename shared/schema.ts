...
export const quotationFormSchema = z.object({
  ...
  description: z.string().optional(),
  totalSellingPrice: z.number().min(0, "Total selling price must be a positive number").default(0),
  totalDiscountedPrice: z.number().min(0, "Total discounted price must be a positive number").default(0),
  totalInstallationCharges: z.number().min(0, "Total installation charges must be a positive number").default(0),
  installationHandling: z.number().min(0, "Installation & handling must be a positive number").default(0),
  globalDiscount: z.number().min(0, "Global discount must be a positive number").default(0),
  gstPercentage: z.number().min(0, "GST percentage must be a positive number").default(18),
  gstAmount: z.number().min(0, "GST amount must be a positive number").default(0),
  finalPrice: z.number().min(0, "Final price must be a positive number").default(0),
